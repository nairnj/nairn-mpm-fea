...
#pragma mark CSTriangle:Constructors and Destructor

#ifdef MPM_CODE

// Main MPM constructor - just sets extra node to match starting node
CSTriangle::CSTriangle(int eNum,int *eNode) : Linear2D(eNum,eNode)
{
...
// Main FEA constructor - passes on to Linear 2D But also sets extra node to match starting node
...
#pragma mark CSTriangle:methods
...
#pragma mark CSTriangle:MPM only methods
...
{
	throw "Triangular elements do not have shape function in MPM code";
}

// Not yet used in MPM
void CSTriangle::ShapeFunction(Vector *xi,int getDeriv,double *sfxn,
...
#pragma mark CSTriangle:FEA only methods
