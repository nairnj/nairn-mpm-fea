...
For a locally adiabatic process, $dq=0$ for material-specific mechanisms, but the temperature might increase. Let $dT_{ad}$ be the temperature change that happens during an adiabatic process in the material model. From above energy flow and adiabatic term from thermoelasticity, which happens even in elastic materials with no energy disspation, is:
\begin{equation}
    dT_{ad}^0 = T_0 {\tens{M}\cdot d\et\over \rho C_V}
\end{equation}
For an isotropic material, $\tens{M}\cdot d\et = - 3K\alpha \Delta V/V$ leading to
\begin{equation}
    dT_{ad}^0 = -T_0 {3K\alpha\over \rho C_V}{\Delta V\over V} = -T_0 J {K\over K_0} \gamma_0 {\Delta V\over V} = -T_0 {K\over K_0} \gamma_0 {\Delta V\over V_0}
\end{equation}
where $J = V/V_0 = \rho_0/ \rho$ and
\begin{equation}
    \gamma_0 = {3K_0\alpha\over \rho_0 C_V}      \label{gamzdef}
\end{equation}
This result assume thermal expansion coefficient is a constant.
...
\end{equation}
Again, the second, isentropic term is subtracted from entropy, but is added back on the next time step (thereby restoring net $dS=0$ for the adiabatic mechanism). If internal energy is needed, it is
\begin{equation}
      U = A + TS
\end{equation}
...
\begin{equation}
    dU = {1\over \rho}\thinspace \st\cdot d\et - C_V dT_{ad}^0 - d\Phi + C_V dT = {1\over \rho}\thinspace \st\cdot d\et + C_V(dT-dT_{ad})
\end{equation}
where $dT=dT_{ext}$ in this case and $dT_{ad}^0$ is adiabatic temperature rise by mechanisms other that material energy dissipation. Here $C_V dT_{ad}^0+d\Phi$ has been equated to $C_VdT_{ad}$; {\rm i.e.}, $dT_{ad}$ now means the temperature rise that would have happened if the process was adiabatic. The $dU$ update is the same for both locally adiabatic and isothermal processes. Similarly, separate tracking of $w$ and $q$ (see above) is the same.

For tracking Helmholtz energy, $-Td_eS = C_V(dT_{ad}  -  dT)$ (because the dissipated energy leaves the object in the form of heat), which gives
\begin{equation}
...
\begin{equation}
    dS = d_iS + {C_V(dT-dT_{ad})\over T} 
\end{equation}

For ``isolated'' system and particle simulations (that is also reversible), the net results will be
\begin{equation}
    dA = {1\over \rho}\thinspace \st\cdot d\et - S dT, \qquad dS = -{C_V dT_{ad}\over T}  \qquad {\rm and}\qquad U = A + TS
\end{equation}
If the process is also elastic
\begin{equation}
    dA = {1\over \rho}\thinspace \st\cdot d\et ,\qquad dS = -{C_V dT_{ad}^0\over T}  \qquad {\rm and}\qquad U = A + TS
\end{equation}
If the process is elastic and reversible, but has ``nonisolated'' particles and system:
\begin{equation}
    dA = {1\over \rho}\thinspace \st\cdot d\et - S dT  \qquad {\rm and}\qquad dS = {C_V (dT-dT_{ad}^0)\over T}
\end{equation}
If $C_V$ is constant and taking $dT_{ad}^0=0$, these can be integrated
\begin{eqnarray}
   S & = & C_V \ln {T\over T_0} \\
...
\end{equation}
This result is general; although individual time steps will be out of balance ({\em e.g.}, $dq\ne0$ when adiabatic), the cumulative heat flow will be correct. Special cases for ``isolated'' particles ({\em i.e.}, $dT_{ext}=0$) are as follows. For locally adiabatic particles is
\begin{equation}
    dw = {1\over \rho}\thinspace \st\cdot d\et, \qquad dq = 0  \qquad {\rm and} \qquad U = w
\end{equation}
For locally isothermal particles is
\begin{equation}
    dw = {1\over \rho}\thinspace \st\cdot d\et, \qquad dq = -C_V dT_{ad}  \qquad {\rm and} \qquad U = w+q
\end{equation}
Here heat is balanced within each time step. All these updates assume the material constitutive law never changes particle temperature directly. When developing materials, particle temperature changes should only be done by calling {\tt Incre\-ment\-Heat\-Energy()} in the base material class. This method updates $q$, handles all special cases, and logs heat energy for subsequent changes in particle temperature, as appropriate.
...
\begin{eqnarray}
      ...
      dS & = & \left\{ \begin{array}{ll}
                ...
                d_iS + {C_V (dT-dT_{ad})\over T} & {\rm locally\ isothermal}
                \end{array} \right. \\
       U & = & A + TS
\end{eqnarray}
The reason the entropy updates differ for locally adiabatic and isothermal are to improve accuracy of the adiabatic result cancel the increment in the next time step with the one in the previous step; otherwise the same update could be used.
