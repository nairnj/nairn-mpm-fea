...
\begin{equation}
     \beta_{eff} = {\rho_0 \gamma_0 C_V\over K} \qquad{\rm or}\qquad \gamma_0 = {K\beta_{eff}\over \rho_0 C_V}
\end{equation}
The isoentropic temperature rise (using $J=\rho_0/\rho$) becomes
\begin{equation}
     dT_{dq=0} = - {K\beta_{eff}T\over \rho C_V}  \left({V(t+\Delta t)-V(t)\over V} \right)    \label{mgdt}
\end{equation}
The heating rate ($dq/dt$) per unit volume becomes
\begin{equation}
    {dq\over dt} = \rho C_V dT_{dq=0} = -K \beta_{eff}T \left({V(t+\Delta t)-V(t)\over V\Delta t} \right) = -T {\tens{M}}\cdot {d\vec{\varepsilon}\over dt} 
\end{equation}
where $\tens{M}=K\vec{\beta}_{eff}$ is the stress-temperature tensor. This result is the standard result for energy flow in small-strain thermoelasticity for an elastic material ({\em e.g.}, see linear thermolasticity by D.~E.~Carlson).
...
\end{equation}
For a monatoic ideal gas $\gamma_0=2/3$; for a diatomic gas $\gamma_0=2/5$; for both $\gamma_0=\gamma-1$. This result is identical to elasticity theory and Eq.~(\ref{mgdt}) by using $K=P$, $\beta_{eff}=1/T$ and $C_V = (3/2)nR/(\rho V)$ (or 5/2 for diatomic gas).
