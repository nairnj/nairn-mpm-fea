...
double Nonlinear2Hardening::GetYield(MPMBase *mptr,int np,double delTime)
{
	return yldred*(1.+beta*pow(alpint,npow)) ;
}
...
double Nonlinear2Hardening::GetKPrime(MPMBase *mptr,int np,double delTime)
{
	return TWOTHIRDS*yldred*beta*npow*pow(alpint,npow-1) ;
}
...
double Nonlinear2Hardening::GetK2Prime(MPMBase *mptr,double fnp1,double delTime)
{
	return SQRT_EIGHT27THS*yldred*yldred*beta*npow*(1.+beta*pow(alpint,npow))*pow(alpint,npow-1)*fnp1;
}

#pragma mark NonlinearHardening::Accessors

// hardening law name
const char *Nonlinear2Hardening::GetHardeningLawName(void) { return "Nonlinear hardening 2"; }
