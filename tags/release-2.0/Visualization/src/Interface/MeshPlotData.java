...
public class MeshPlotData  extends PlotControl implements MouseMotionListener
{
	...
	{	// skip until something is loaded
		...
		// coordinates
		xlabel.setText("x: "+MoviePlotWindow.formatDouble(pt.x));
		ylabel.setText("y: "+MoviePlotWindow.formatDouble(pt.y));
		
		// can't check while movie is running
		if(resDoc.docCtrl.movieFrame.isMovieRunning())
		...
		{	elemlabel.setText("el: "+currentElem);
			...
			{	MaterialPoint mpt=resDoc.mpmPoints.get(0);
				...
				zlabel.setText("z: "+MoviePlotWindow.formatDouble(nearmp.getPlotValue()));
			}
			else if(plotView.getPlotComponent()!=PlotQuantity.MESHONLY)
			{	mplabel.setText("mp: ---");
				ElementBase cElem=resDoc.elements.get(currentElem-1);
				zlabel.setText("z: "+MoviePlotWindow.formatDouble(cElem.getValueAt(pt)));
			}
			else
			{   zlabel.setText("z: ---");
				mplabel.setText("mp: ---");
