...
double ExponentialSoftening::GetDDelta(double de,double e0,double delta,double gScaling) const
{   double k = 1./(Gc*gScaling);
    double x = k*e0*exp(-k*delta);
    double ddelta = de + (x + gsl_sf_lambert_W0(-x*exp(-k*de-x)))/k;
    
//  double ddelta2 = SofteningLaw::GetDDelta(de,e0,delta,gScaling);
