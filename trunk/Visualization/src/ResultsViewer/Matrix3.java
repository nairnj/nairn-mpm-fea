...
public class Matrix3
{
	...
	public Matrix3 Eigenvectors(Vector3d Eigenvals)
	{
		Matrix3 Eigenvecs = new Matrix3();
		
	    if(is2D)
		{	double [] vals = dsyev2(m[0][0],m[0][1],m[1][1],Eigenvals.x,Eigenvals.y);
			Eigenvals.x = vals[0];
			Eigenvals.y = vals[1];
		...
		{	// set to 3D matrix
			...
			A[1][2] = m[1][2];
			A[2][2] = m[2][2];
			dsyevq3(A, Q, w);			// QL - if finds eigenvalues
			Eigenvecs.set(Q);
			Eigenvals.x = w[0];
			Eigenvals.y = w[1];
			Eigenvals.z = w[2];
	...
	public Matrix3 LeftDecompose(Matrix3 R)
	{
	    ...
	    Vector3d Eigenvals = B.Eigenvalues();
	    double lam1 = Math.sqrt(Eigenvals.x);
	    double lam2 = Math.sqrt(Eigenvals.y);
	    double lam3 = Math.sqrt(Eigenvals.z);
	...
	// ----------------------------------------------------------------------------
	private int dsyevv3(double A[][], double Q[][], double[] w)
	{
		...
		if ((t=Math.abs(w[2])) > wmax)
			wmax = t;
		double sqr = 8.0 * mpmEpsilon * wmax;
		thresh =sqr*sqr;
		
		// Prepare calculation of eigenvectors
		n0tmp   = A[0][1]*A[0][1] + A[0][2]*A[0][2];
		n1tmp   = A[0][1]*A[0][1] + A[1][2]*A[1][2];
		...
		else if (norm < (64.0 * mpmEpsilon)*(64.0 * mpmEpsilon) * error)
		...
		t = w[0] - w[1];
		if (Math.abs(t) > 8.0 * mpmEpsilon * wmax)
		{
		...
		else
		{
			...
			for (i=0; i < 3; i++)
			{
				...
				if (n0 > thresh)
				{
					...
					norm     = Q[0][1]*Q[0][1] + Q[1][1]*Q[1][1] + Q[2][1]*Q[2][1];
					if (norm > (256.0 * mpmEpsilon)*(256.0 * mpmEpsilon) * n0) // Accept cross product only if the angle between
