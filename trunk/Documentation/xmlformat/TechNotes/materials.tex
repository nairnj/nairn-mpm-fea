...
The ideal gas material implements ideal gas law as a hyperelastic material. It seems to work well for gas confined within a solid or constrained by rigid particles. It does not handle gas dynamics such as irreversible free expansion, but does handle reversible processes including coupled conversion of energy into heat ({\em i.e.}, cooling on expansion and heating on compression).

The ideal gas law is
\begin{equation}
    PV = nRT
\end{equation}
The ideal gas properties are defined by picking any reference pressure, $P_0$, reference temperature, $T_0$, and reference density, $\rho_0$. If $M$ is the molecular weight of the gas molecules, the reference density is given by
\begin{equation}
...
\end{equation}
where $J = \det \F = V/V_0$. The Cauchy stress due to this pressure is $-P\I$, but the code needs a specific Kirchoff stress normalized to $\rho_0$ or
\begin{equation}
    \vec\tau^{(s)} = -{PJ\over \rho_0}\I  = - {P_0\over \rho_0} {T\over T_0}\I
\end{equation}

In code, an incremental approach is often preferred. 
If $\tau_n^{(s)}$ is any diagonal element of the specific Kirchoff stress tensor, then
\begin{equation}
   \tau_{n+1}^{(s)} = - {P_0\over \rho_0} {T_n\over T_0} {T_{n+1}\over T_n}
...
\begin{equation}
    {dU\over \rho_0 V_0} = - {1\over2} {P_n+P_{n+1}\over \rho_0} {V_{n+1}\over V_0}
...
\begin{equation}
     \tau_i^{(s)} = - {P_0\over \rho_0} {T\over T_0}
\end{equation}
