...
We only need to evaluate terms lis
\begin{equation}
...
\end{equation}
where $\Delta \e{i} = \Delta t (d\e{}(i\Delta t)/dt) = \e{i+1}-\e{i}$. Evaluating the integral by the midpoint rule leads to
\begin{equation}
...
Returning to 3D (and plane strain) equations, the process is to define a history variable for each component of deviatoric strain and for each relaxation time such as
\begin{equation}
      h_{xx,k}^{(i)} = \sum_{j=1}^{i}  -{G_k\over\tau_k} e^{-j\Delta t/\tau_k}\Delta e_{xx,i-j+1}
\end{equation}
and similarly for $h_{yy,k}^{(i)}$,  $h_{zz,k}^{(i)}$ (3D), $h_{xy,k}^{(i)}$,  $h_{xz,k}^{(i)}$ (3D),  and $h_{yz,k}^{(i)}$ (3D). Then following strain increments of $d\e{xx}$, $d\e{yy}$,  $d\e{zz}$ (3D), $d\g{xy}$, $d\g{xy}$, and $d\g{xy}$, $de_{ii}=d\e{ii} - Tr(\et)/3$, $de_{xy}=d\g{xy}/2$, $de_{xz}=d\g{xz}/2$, and $de_{yz}=d\g{yz}/2$, the stress increments are
\begin{eqnarray}
      dP & = & -K(d\e{xx} + d\e{yy} + d\e{zz} - 3\er{}) \\
      ds_{ij} & = & \left(G_e + {\Delta t\over 2}G_e'\right) de_{ij}
                          + \Delta t  \sum_{k=1}^n h_{ij,k}^{(i-1)} 
\end{eqnarray}
The history variables update using
\begin{equation}
        h_{ij,k}^{(i)}  = e^{-\Delta t/\tau_k}\left(h_{ij,k}^{(i-1)} - {G_k\over\tau_k}de_{ij}\right)  
\end{equation}
Note that in plane strain modeling that $d\e{zz}$ is always zero, but $de_{zz} = - Tr(\et)/3$ so $h_{zz,k}^{(i)} \ne 0$. In general, because $\et$ is traceless, $h_{xx,k}^{(i)}+h_{yy,k}^{(i)}+h_{zz,k}^{(i)}=0$, which means instead of tracking $h_{zz,k}^{(i)}$, it can always be found from $-(h_{xx,k}^{(i)}+h_{yy,k}^{(i)})$.

An alternate formulation, although equivalent, in Simo and Hughes begins with Eq.~(\ref{stressterm}) and directly substitutes exponential expansion to get:
\begin{equation}
     \sigma(t) = G_0\e{}(t) + \sum_k^n G_k \int_0^t e^{-(t-\tau)/\tau_k} {d\e{}\over d\tau} d\tau
\end{equation}
Next, introduce the internal varable, $\alpha_k$, that satisfied
\begin{equation}
...
\begin{equation}
      \alpha_k = \int_0^t e^{-(t-\tau)/\tau_k} {\e{}(\tau)\over \tau_k}d\tau = \e{}(t) - \int_0^t e^{-(t-\tau)/\tau_k} {d\e{}(\tau)\over d\tau}d\tau
\end{equation}
Substitution into the stress terms give
\begin{equation}
     \sigma(t) = G_0\e{}(t) + \sum_k^n G_k \e{}(t) - \sum_k^n G_k \alpha_k = G_e\e{}(t) -  \sum_k^n G_k \alpha_k
\end{equation}

A stress increment is now found from
\begin{equation}
    \Delta\sigma_i = G_e\Delta\e{i} - \sum_k^n G_k \Delta\alpha_{i,k}
\end{equation}
where $\Delta\alpha_{i,k} = \alpha_{i+1,k}-\alpha_{i,k}$. By midpoint integration (with $\e{0}=0$):
\begin{equation}
     \alpha_{i,k} = {\Delta t\over \tau_k}\left[ \sum_{j=1}^{i-1} e^{-(i-j)\Delta t/\tau_k} \e{j}  + {\e{i}\over 2}\right]
\end{equation}
Therefore
\begin{equation}
...
\begin{equation}
     \Delta\alpha_{i,k} = {\Delta t\over \tau_k}\left[ \sum_{j=1}^{i} e^{-(i+1-j)\Delta t/\tau_k} \Delta\e{j-1}    
             + {\Delta\e{i}\over 2}\right]
\end{equation}
Reversing the order of the summation give
\begin{equation}
