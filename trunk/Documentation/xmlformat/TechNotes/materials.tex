...
\end{equation}
where $\dev$ is the deviatoric stress tensor and $K(\alpha)$ defines the tensile yield stress as a function of the hardening variable and possibly other variables ({\it e.g.}, plastic strain rate or temperature, but not pressure). All materials that fit this mold are handled in {\tt NairnMPM} by the {\tt IsoPlasticity} class. A new material that only has a different hardening law only needs code for that hardening law by subclassing {\tt HardeningLawBase} class. The only materials that need to subclass {\tt IsoPlasticity} is if they need a different equation of state to handle elastic parts differently.

In terms of this  deviatoric stress\begin{equation}
...
\end{equation}
where $ \dev_{trial}$ is the deviatoric stress calculated be assuming no plastic deformation. By noting that $\dev=\|\dev\|\ndev$ and rearranging
\begin{equation}
