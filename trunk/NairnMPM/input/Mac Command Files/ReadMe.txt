...
The later may be more current due to delays in getting updates into the Mac App store and to new Apple security rules that might make applications that do interesting things no longer permitted. Apple's new thinking is that using your computer for calculations (which in NairnFEAMPM involves writing results to disk) is too risky for ordinary users. Thus power users should go the the above web site instead of the Mac App store.

Best use of these files is to:

1. Open the copied file in NairnFEAMPM
2. Change any options in the files (as documented in comments)
3. Run the calculations and save the outputs to a folder outside the project hierarchy.
