...
void NonlinearHardening::PrintYieldProperties(void)
{
...
double NonlinearHardening::GetYield(MPMBase *mptr,int np,double delTime)
{   
	return yldred*pow(1.+beta*alpint,npow) ;
}
...
double NonlinearHardening::GetKPrime(MPMBase *mptr,int np,double delTime)
{
	return TWOTHIRDS*yldred*beta*npow*pow(1.+beta*alpint,npow-1) ;
}
...
double NonlinearHardening::GetK2Prime(MPMBase *mptr,double fnp1,double delTime)
{
	return SQRT_EIGHT27THS*yldred*yldred*beta*npow*pow(1.+beta*alpint,2.*npow-1)*fnp1;
}

#pragma mark NonlinearHardening::Accessors

// hardening law name
const char *NonlinearHardening::GetHardeningLawName(void) { return "Nonlinear hardening"; }
