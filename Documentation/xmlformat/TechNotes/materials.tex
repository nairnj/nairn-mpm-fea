...
\begin{equation}
       \nabla u = \left(\begin{array}{cc}
               \left[\begin{array}{cc}
                 ...
                 \end{array}\right)
\end{equation}
where $du_{ij} = \partial u_i/\partial j$ ($du_{zz}=0$ for plain strain, but not for plane stress or axisymmetry). If $\tens A$ is the $2\times2$ partition, then
\begin{equation}
...
\end{equation}
where $\beta_{2,i} = c_i$, $\beta_{k,0}=c_0\beta_{k-1,1}$, and $\beta_{k,1}=c_1\beta_{k-1,1} + \beta_{k-1,0}$. In other words, $\tens I$ and $\tens A$ are a basis for all powers of $\tens A$. Finally, we can expand the incremental deformation gradient using $k_{max}$ terms as
\begin{equation}
...
\end{equation}
An efficient computer algorithm for all non-zero elements of the deformation gradient is
\begin{verbatim}
    double c0 = duxy*duyx - duxx*duyy, c1 = duxx + duyy;
    double alpha0 = 1. + 0.5*c0, alpha1 = 1. + 0.5*c1.;
    double alphaz = duzz*duzz; dFzz = 1. + duzz + 0.5*alphaz
    
    // loop only needed if kmax>2
    int k,kmax=5;
...
\end{verbatim}
Each extra term included in the expansion costs 7 multiplications/division and 4 additions. Direct matrix multiplication would add 14 multiplcations/division and 13 additions for each term.
