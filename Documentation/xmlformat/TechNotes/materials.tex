...
Finally, table \ref{tdmodes} summarized thermodynamics properties of the various {\tt NairnMPM} modes. The global terms are defined by:
\begin{equation}
     dQ = \sum_p dq, \quad dS = \sum_p S_p, \quad T = {1\over n_p}\sum_p T_p, \quad d_cS = \sum_p {dq_{conduction}\over T_p}
\end{equation}
An `` isolated'' system means no thermal boundary conditions are being uses, while a ``nonisolated'' system has thermal boundary conditions. ``Isolated'' particles means conduction is turned off, while ``nonisolated'' particles means conduction is on. ``Adiabatic'' means energy coupling is turned on, while ``Isothermal'' means energy coupling is off.


\begin{table}\renewcommand{\arraystretch}{1.2}
\caption{The changes in particle heat ($dq$), global heat ($dQ$), global entropy ($dS$), irreversible entropy due to heat conduction ($d_cS$), and average temperature ($dT$) for each {\tt NairnMPM} thermodynamics mode. For entropy, all processes other than heat conduction are assumed reversible.}
\vskip6pt
\label{tdmodes}
\centering
\begin{tabular}{|c|c|c|c|}
\hline
System & Particles & Adiabatic & Isothermal \\
\hline
Isolated & Isolated & $dq=dS=dQ=d_cS=0$$\thinspace^1$ & $dq=-d\Phi$$\thinspace^1$, $dS\ne0$, $dQ\ne0$ \\
 &  & $dT\ne0$ & $d_cS=0$, $dT=0$\\
\hline
Isolated & Nonisolated & $dq=C_V (dT - dT_{ad})$$\thinspace^2$, $dQ=0$ & $dq=C_V (dT - dT_{ad})$, $dQ\ne0$ \\
 &  & $dS=d_cS\ge0$, $dT\ne0$ & $dS\ne0$, $d_cS\ge0$, $dT=0$$\thinspace^3$\\
\hline
Nonisolated & Isolated & $dq=C_V (dT - dT_{ad})$, $dQ\ne 0$& $dq=C_V (dT - dT_{ad})$, $dQ\ne0$\\
 &  & $dS\ne0$, $d_cS=0$, $dT\ne0$ & $dS\ne0$, $d_cS=0$, $dT\ne0$$\thinspace^4$\\
\hline
Nonisolated & Nonisolated & $dq=C_V (dT - dT_{ad})$, $dQ\ne 0$ & $dq=C_V (dT - dT_{ad})$, $dQ\ne0$\\
 &  & $dS\ne0$, $d_cS\ge0$, $dT\ne0$ & $dS\ne0$, $d_cS\ge0$, $dT\ne0$\\
\hline
\end{tabular}
...
time step will add $C_V(dT_i-dT_0)$ to the particle heat energy. The above conditions will hold thereafter, but a
constant will be added to $dS$ and $dQ$.
\item The net results will be that $dq = dq_{conduction}$.
\item This mode is identical to isolated particles and system if the initial temperature of all particles is equal to the stress-free temperature. Thus, a simulation in this mode should turn off conduction for efficiency when all particles
are at the stress free temperature.
\item The only boundary condition possible here is an thermal ramp that applies uniform temperature change to all particles.
\end{enumerate}
}
\end{table}
