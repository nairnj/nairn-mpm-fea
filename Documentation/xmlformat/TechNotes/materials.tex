...
The differential in internal energy per unit mass, $U$, with dissipative and irreversible processes is (Fung, page 347)
\begin{equation}%
      dU = dw + dq = {1\over \rho}\thinspace \st\cdot d\et + T \thinspace dS_e
\end{equation}%
where $dw$ is work and $dq$ us heat exchange with the exterior (or in the case of position-dependent energies, exchange with surrounding area due to heat conduction. The second form associates work with stress power or strain energy, where $d\et$ is the incremental strain tensor (relative to a stress free state if there are residual strains), and heat flow with $dq = T \thinspace dS_e$ where $dS_e$ is the change in entropy (per unit mass) due to exchange of energy with the exterior. The full change in entropy per unit mass is
\begin{equation}%
      dS = dS_e + dS_i = {C_V dT_{ext}\over T} + dS_i
\end{equation}%
where $dS_i\ge0$ is entropy change per unit mass due to internal irreversible processes that do not exchange energy with exterior. (Note: it is possible the total $dS$ would differ for materials such as rubbers, but I think this approach is good for most materials in computational mechanics.) 
...
with differential
\begin{equation}%
    dA = dU - TdS - SdT = {1\over \rho}\thinspace \st\cdot d\et + T dS_e - TdS - SdT
              = {1\over \rho}\thinspace \st\cdot d\et  - SdT - T dS_i
\end{equation}%

The goal of {\tt NairnMPM} is to track the most appropriate energies (see below). The challenge is dealing with material-specific dissipation or other isoentropic temperature changes while also allow for external heating through thermal boundary conditions and heat conduction. A given simulation can be described as locally adiabatic or isothermal. It is locally adiabatic if mechanical energy coupling is activated (where mechanical energy coupling means dissipated energy is converted to heat). It is locally isothermal when mechanical energy is off, which implies dissipated energy is lost to the exterior.

\subsection{Locally Adiabatic}

For a locally adiabatic process, $dq=0$ for material-specific mechanisms, but the temperature might increase. Let $dT_S$ be the temperature changes that happen at constant $S$ in the material model. In general problems, $T$ might also increase due to exchange of heat with the exterior through conduction. Let those temperature changes be $dT_{ext}$. The change in internal energy become
\begin{equation}
    dU = {1\over \rho}\thinspace \st\cdot d\et + C_V dT_{ext}
\end{equation}
But in MPM, the $dT$ on any given time step will be $dT_S+dT_{ext}$ and there is no way to separate them for $dT$ input alone. One solution is to separate them in the particle strain update where $dT_S$ is known:
\begin{equation}
    dU = {1\over \rho}\thinspace \st\cdot d\et - C_V dT_S + C_V (dT_S+dT_{ext}) = {1\over \rho}\thinspace \st\cdot d\et + C_V(dT-dT_S)
\end{equation}
In a simulation, $dT_S$ can be calculated for a given time step and subtracted from $U$. It will be added back on the next time step through the contribution of $dT_S$ to $dT$. For example, plasticity will dissipate energy $d\Phi$, which will cause temperature change of $dT_S = d\Phi/C_V$. A simulation might prefer to assume that only of fraction of $d\Phi$ leads to heat causing temperature rise. The remained mist be lost to the exterior as heat flow. In this case, the update should still subtract to the total $d\Phi = C_V dT_S$ and the portion that causes temperature rise will be added back in the next time step and the portion that is lost to the exterior will be correctly added as heat flow.

Instead of tracking only total $U$, a material could track total work, $w$, and heat flow, $q$, using updates
\begin{equation}
     dw = {1\over \rho}\thinspace \st\cdot d\et \qquad {\rm and} \qquad dq = C_V(dT-dT_S)
\end{equation}
When needed, $U = w + q$.


For tracking Helmholtz energy, $T dS_e = dq = 0$, giving
\begin{equation}
    dA = {1\over \rho}\thinspace \st\cdot d\et + C_V(dT-dT_S) - S dT - T dS_i
\end{equation}
This update needs to entropy, which therefore must be tracked separately (using $dS$ from above), but the problem again is that $dT_{ext}$ is not known. A solution similar to that used for internal energy is
\begin{equation}
    dS = dS_i + {C_V dT\over T} - {C_V dT_S\over T + dT_S}
\end{equation}
Again, the second, isentropic term is subtracted from entropy, but is added back on the next time step (thereby restoring $dS=0$ for that term).

If there is no external heating and the process is reversible, the net results will be
\begin{equation}
    dA = {1\over \rho}\thinspace \st\cdot d\et  \qquad{\rm and} \qquad dS = 0
\end{equation}
and this result holds for both elastic and plastic materials. If internal energy is needed, it is
\begin{equation}
      U = A + TS
\end{equation}
For the reversible process with no external heating, $U=A$.

\subsection{Locally Isothermal}

For a locally isothermal process, $dT=0$ due to material processes, although $dT_{ext}$ may still be non zero. if $d\Phi$ is material dissipated energy, then the internal energy change is
\begin{equation}
    dU = {1\over \rho}\thinspace \st\cdot d\et - d\Phi + C_V dT = {1\over \rho}\thinspace \st\cdot d\et + C_V(dT-dT_S)
\end{equation}
where $dT=dT_{ext}$ in this case. Here $d\Phi$ has been equated to $C_VdT_S$, but $dT_S$ now means the temperature rid that would have happened if the process was adiabatic. The $dU$ update is the same for both locally adiabatic and isothermal processes. Similarly, separate tracking of $w$ and $q$ (see above) is the same.

For tracking Helmholtz energy, $-TdS_e = d\Phi - C_V dT$ (because the dissipated energy leaves the object in the form of heat), which gives
\begin{equation}
    dA = {1\over \rho}\thinspace \st\cdot d\et - S dT - T dS_i
\end{equation}
The Helmholtz update therefore depends on the simulation type. The difference is the $C_V(dT-dT_S)$ term that is included for locally adiabatic, but omitted for locally isothermal. The entropy update is similar, but should account for isothermal state to be
\begin{equation}
    dS = dS_i + {C_V dT-d\Phi\over T} 
\end{equation}

If the process has no external heating and is reversible, the net increments are
\begin{equation}
...
\end{equation}
If the process is elastic and reversible, but has external heating, then
\begin{equation}
...
\end{equation}%
Substituting the above two results into Eq.~(\ref{GreensA}) leads to path-independent result (which includes path integral and area integral) of
\begin{equation}%
...
\end{equation}%
where $\Gamma^*$ is an open path that starts on one surface of the crack and ends on the other and $A^*$ is the area bounded by $\Gamma^*$ and the crack surface.
...
or
\begin{equation}%
           \int_\Gamma \left(\rho\thinspace U\thinspace dy -  T_i {\partial u_i\over \partial x} ds\right)
                        - \int_{A^*}  {dq\over dx} \thinspace  dx \thinspace dy = 0
\end{equation}%
or, for an adiabatic process,
\begin{equation}%
      J = \int_{\Gamma^*} \left(\rho\thinspace U\thinspace dy -  T_i {\partial u_i\over \partial x} ds\right)
\end{equation}%
These results seem to hold for irreversible processes as well.

In MPM, the calculations need tracking of $U$ and $q$. The needed gradients of $q$ can be found by gradient extrapolation to the grid.
...
\begin{equation}
    dU = {1\over \rho}\thinspace \st\cdot d\et + C_V (dT - dT_S)
\end{equation}

\item Track work and heat energy using
\begin{equation}
    dw = {1\over \rho}\thinspace \st\cdot d\et, \qquad dq = C_V (dT - dT_S)  \qquad {\rm and} \qquad U = w+q
\end{equation}

\item Track Helmholtz energy and entropy using
\begin{eqnarray}
      dA & = & \left\{ \begin{array}{ll}
                {1\over \rho}\thinspace \st\cdot d\et + C_V(dT-dT_S) - S dT - T dS_i & {\rm locally\ adiabatic} \\
                {1\over \rho}\thinspace \st\cdot d\et - S dT - T dS_i & {\rm locally\ isothermal}
                \end{array} \right.\\
      dS & = & \left\{ \begin{array}{ll}
                dS_i + {C_V dT\over T} - {C_V dT_S\over T + dT_S} & {\rm locally\ adiabatic} \\
                dS_i + {C_V dT-d\Phi\over T} & {\rm locally\ isothermal}
                \end{array} \right. \\
       U & = & A + TS
\end{eqnarray}
...
The advantage of \#1 is that only one item needs to be tracked, but then only one item is known. For example, that approach would not have enough information for the full thermodynamic $J$ integral. The advantage of \#2 is that it has simple updates and can be used for $J$ integral calculations. The disadvantage is the neither $w$ nor $q$ are state functions, which means $A$ and $S$ are not known. The advantage of \#3 is that $A$ and $S$ are state function and if needed they could be used to find $U$ as well. The disadvantage is the the equations are more complex and require different updates for locally adiabatic and isothermal. The current version of {\tt NairnMPM} tracks $w$ in strain energy, $q$ in heat energy, and for plasticity materials tracks $\Phi$ due to plasticity in plastic energy. Future materials development may choose to track $A$ in strain energy and $S$ in heat energy and when needed calculate internal energy. Materials that adopt this approach, however, would not be able to support features that need heat energy, unless they separately track that as well in a history variable.
...
\end{equation}
The stresses and energy are found using $\F^*$ instead of $\F$ which is now deformation for the free expansion volume instead of from the initial volume. The net effects are that elements of $\F$ are divided by $\lambda_{res}$ and stress should be found using $J_{eff} = J/\lambda_{res}^3$ and $\B_{eff} = \B/\lambda_{res}^2$.

When doing incremental deformation, $\F_{k+1} = \tens{dF}\thinspace\F_k$ and incremental volume ratio is $dJ = \tens{dF} = V_{k+1}/ V_k$. $J_{eff}$ is $V/V_{sf}$ where $V_{sf}$ is the current stress free volume and it is related to $J$ (as defined above) using
\begin{equation}
  J_{eff} = {V\over V_0} {V_0\over V_{sf}} = {J\over J_{res}} = {J\over \lambda_{res}^3}
\end{equation}
For incremental deformation, $J_{k+1} = dJ J_k$, but we really want to increment $J_{eff,k+1} = dJ_{eff}J_{eff,k}$, which is\begin{equation}
...
A good when to track internal energy in MPM on each time step is to use
\begin{equation}
      dU = \vec\sigma\cdot\deff - C_V dT_{dq=0} - d\Phi + C_V dT
\end{equation}
The first term is total internal energy change due to Mie-Gr\"{u}niesen equation of state and plastic deformation. The second and third terms subtract the isentropic temperature rise and dissipated plastic energy. The last term is thermal energy on the current time step. Note that the isoentropic and dissipated energy are not removed; they will appear on the next time step because they will contribute to the $C_VdT$ term. Actually the isoentropic temperature rise does not change internal energy. In this energy tracking, that term is subtracted on one time step and added back in on the next to have net zero affect.
Note that if a simulation is adiabatic and there are no external heat sources, a simpler update could be
\begin{equation}
      dU = \vec\sigma\cdot\deff
\end{equation}
But the previous method works in this case too, with the only difference be that the addition some energy is delayed until the next time step.  
...
In {\tt NairnMPM}, the cumulative work is tracked in the particle's plastic energy. The particle's strain energy tracks total internal energy in the gas, which for an ideal gas is
\begin{equation}
   U = C_V(T-T_0) + U_0 \qquad {\rm or\ incrementally} \qquad  dU = C_V dT
\end{equation}
That energy will depend on the analysis options. The default option is isothermal such that work is done, but there is no temperature rise. The total internal energy change will be zero, which implies all the work is lost as heat. The particle's plastic energy will give the work done and the internal energy (in particle's strain energy) will be zero. The alternate option is to turn mechanical energy option on (with or without conduction). In this mode all the work will be input as heat that will cause a temperature rise. The internal energy (in particle's strain energy) will increase linearly with temperature and the cumulative work (equal to cumulative heat) will be in the particle's plastic energy.
...
When doing conduction and/or energy coupling, this material needs heat capacity and thermal conductivity. Heat capacity is calculated using ideal gas law theory ($C_V=(3/2)nR/(\rho_0 V_0)$ for monotonic gas and $C_V=(5/2)nR/(\rho_0 V_0)$ for diatomic gas in J/(kg-K)). Thermal conductivity, is needed. The current code assumes conductivity is a temperature-independent property (as entered), although conductivity of a gas does vary with temperature. If simulations with large temperature changes of the gas are important, this material will need to be improved to allow temperature-dependent thermal conductivity.

To find heat capacity from input parameters, substitute $nR = P_0V_0/T_0$ at reference conditions to get
\begin{equation}
     C_V =  {3\over 2} {P_0 \over \rho_0T_0}
\end{equation}
for monatomic gas (or replace $3/2$ with $5/2$ for diatomic gas.
