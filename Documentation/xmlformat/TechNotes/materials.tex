...
In MPM, all thermodynamics quantities will vary with position by depending on particle state. In effect, each particle is considered as a ``system'' with a well defined thermodynamics state. Work is done on the particle by stresses and strains and a particle can exchange heat with the system by conduction or by any condition that prescribes particle temperature. Thus the general calculations should account for exchange of heat of each particle with the ``exterior,'' where exterior means other particles or a boundary condition. In some calculations, it might be advantageous to have a special case when no such exchange occurs, denoted here as an ``isolated'' particle simulation.
...
\end{equation}%
where $dw$ is work and $dq$ ss heat exchanged with the ``exterior .'' The second form associates work with stress power or strain energy, where $d\et$ is the incremental strain tensor (relative to a stress free state if there are residual strains), and heat flow with $dq = T \thinspace dS_e$ where $dS_e$ is the change in entropy (per unit mass) due to exchange of energy with the exterior. The full change in entropy per unit mass is
\begin{equation}%
...
For an ``isolated'' particle simulation (that is also reversible), the net results will be
\begin{equation}
...
\end{equation}
where $dT=dT_{ext}$ in this case. Here $d\Phi$ has been equated to $C_VdT_S$, but $dT_S$ now means the temperature rise that would have happened if the process was adiabatic. The $dU$ update is the same for both locally adiabatic and isothermal processes. Similarly, separate tracking of $w$ and $q$ (see above) is the same.
...
\begin{equation}
    dA = {1\over \rho}\thinspace \st\cdot d\et ,\qquad dS = 0  \qquad {\rm and}\qquad U=A
\end{equation}
If the process is elastic and reversible, but has ``nonisolated'' particles
\begin{equation}
...
\end{equation}%
The prior form extends it to irreversible processes (if $dS_i$ can be found).
...
\end{equation}
This result is general. A specific result for ``isolated'' particles  and locally adiabatic is
\begin{equation}
    dw = {1\over \rho}\thinspace \st\cdot d\et, \qquad dq = 0  \qquad {\rm and} \qquad U = w
\end{equation}
Although a special case for ``isolated'' particles  and locally isothermal could be written by using $dT=0$, some material models might have a mechanism that changes particle temperature, despite having mechanical energy coupling off. To handle those materials, this special case should use the general update. A future modification might be a more definitive isothermal mode (rather then just lack of mechanical energy coupling) and then conforming materials would never change the particle's temperature. In this special case the update would be
\begin{equation}
    dw = {1\over \rho}\thinspace \st\cdot d\et, \qquad dq = -C_V dT_S  \qquad {\rm and} \qquad U = w+q
\end{equation}
But this special case it not yet official in {\tt NairnMPM}.
